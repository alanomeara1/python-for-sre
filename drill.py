...
def build_toc(text: str, deep: bool = True) -> str:
    lines = [TOC_START, "", "## Contents", ""]
    seen: dict[str, int] = {}
    in_code = False
    for line in text.splitlines():
        if line.startswith("```"):
            in_code = not in_code          # a ### inside a code block is not a heading
        if in_code:
            continue
        if line.startswith("## ") and not line.startswith("## Contents"):
            heading = line[3:].strip()
        elif line.startswith("### ") and deep:
...
# README lists sections only; PATTERNS lists every card, because that is what you look up.
TOC_FILES = {"README.md": False, "PATTERNS.md": True}


def cmd_toc(args) -> int:
    stale = 0
    for name, deep in TOC_FILES.items():
        path = ROOT / name
        text = path.read_text()
        if TOC_START not in text or TOC_END not in text:
            print(f"--  {name}: no toc markers, skipped")
            continue

        before, rest = text.split(TOC_START, 1)
        _, after = rest.split(TOC_END, 1)
        updated = before + build_toc(text, deep=deep) + after

        if args.check:
            if updated != text:
                print(f"BAD {name} contents list is out of date; run: drill toc")
                stale += 1
            else:
                print(f"ok  {name} contents list is current")
            continue

        path.write_text(updated)
        print(f"{'wrote' if updated != text else 'same '} contents list in {name}")
